...
class EmbeddingsFilter(BaseDocumentCompressor):
    ...
    def compress_documents(
        ...
        # give a unique id to each document chunk with the format "document_id-chunk_id"
        doc_id = -1
        current_doc = None
        current_chunk_id = 0
        for doc in documents:
            if doc.metadata.get("source") != current_doc:
                doc_id += 1
                current_doc = doc.metadata.get("source")
                current_chunk_id = 0
            doc.metadata["chunk_id"] = f"{doc_id}-{current_chunk_id}"
            current_chunk_id += 1
